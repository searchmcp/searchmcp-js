...
import {CountryCode, DateRange, Location, SearchRequestSchema, SearchResponseSchema} from "../src/schema.js";
...
describe("SearchMCPClient", () => {
    ...
    it("defaults numberOfResults to 10 and trims query", async () => {
        ...
        const res = await client.search("   gpu deals   ");
    ...
    it("accepts location as enum key OR value", async () => {
        ...
        await client.search("q", Location.USA);
    });
    ...
    it("throws APIError for non-OK HTTP with normalized response", async () => {
        ...
        await expect(client.search("test")).rejects.toMatchObject({name: "APIError", status: 401});
    });
    ...
    it("retries on 429 then succeeds (exponential backoff)", async () => {
        ...
        try {
            ...
            const p = client.search("rate limit pls");
    ...
    it("retries on 5xx up to maxRetries then throws", async () => {
        ...
        try {
            ...
            const p = client.search("unstable");
            p.catch(() => {});
    ...
    it("times out requests and surfaces APIError", async () => {
        ...
        try {
            ...
            const p = client.search("timeout me");
            p.catch(() => {});
    ...
    it("supports caller AbortSignal", async () => {
        ...
        const p = client.search("abort me", undefined, undefined, 10, undefined, {signal: controller.signal});
        controller.abort();
    ...
    it("sends full body with provided optional params", async () => {
        ...
        const client = new SearchMCPClient({apiKey: API_KEY});
        await client.search("gaming laptop deals", "USA", "US", 25, "PAST_MONTH");
    ...
    it("normalizes unknown thrown errors into APIError", async () => {
        ...
        await expect(client.search("boom")).rejects.toBeInstanceOf(APIError);
        await expect(client.search("boom").catch((e) => e.message)).resolves.toMatch(
    ...
    it("throws APIError when server returns 402 (insufficient credits)", async () => {
        ...
        const client = new SearchMCPClient({apiKey: API_KEY});
        await expect(client.search("credits")).rejects.toMatchObject({name: "APIError", status: 402});
    });

    it("works for minimal valid server response shape", async () => {
        globalThis.fetch = vi.fn(async () => mkRes(200, {status: "SUCCESS"}));
        const client = new SearchMCPClient({apiKey: API_KEY});
        const out = await client.search("ok");
